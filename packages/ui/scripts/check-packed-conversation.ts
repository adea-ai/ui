...
try {
  ...
  for (const condition of ['compiled', 'solid'] as const) {
    const dir = join(consumer, condition)
    mkdirSync(dir)
    writeFileSync(
      join(dir, 'index.html'),
      '<div id="app"></div><script type="module" src="/main.tsx"></script>'
    )
    writeFileSync(join(dir, 'main.tsx'), fixture)
    writeFileSync(
      join(dir, 'style.css'),
    ...
    const css = chunks
      ...
      .join('\n')
    if (gzipSync(code).length > MAX_GZIP_BYTES)
      throw new Error('Packed conversation exceeds measured 26 KiB gzip JS budget')
    if (Buffer.byteLength(css) > MAX_CSS_BYTES)
      throw new Error('Packed conversation exceeds measured 42 KiB raw CSS budget')
