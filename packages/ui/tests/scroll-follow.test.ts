/*
 * Behavior tests translated from KiroCrew website/src/test/FollowController.test.ts,
 * pinned at 283e136c0f902e965a535a7c9548c57c7504fed0.
 * Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
 * Apache-2.0: http://www.apache.org/licenses/LICENSE-2.0
 * React/jsdom globals replaced by controlled descriptor restoration; deterministic
 * grids replace fast-check without adding a dependency. Virtual-anchor tests and
 * source-text coupling checks are excluded from this plain-scroller contract.
 */
...
import {
  ...
  FOLLOW_REENGAGE_PX,
} from '../src/components/conversation/scroll-follow-core'
  ...
  it('does NOT re-engage when the band arrives at a STILL reader', () => {
    ...
    expect(
      resolveUserScrollStick({
        ...
        scrollTop: 590,
        prevScrollTop: 590,
        geom: { scrollTop: 590, scrollHeight: 1000, clientHeight: 400 },
      })
    ).toBe(false)
  })
...
describe('evaluateAutoPin — the race-proof core', () => {
  ...
  describe('readerMovedSinceWrite — who opened the gap', () => {
    ...
    it('never overrides a released stick or an owning restore', () => {
      expect(
        evaluateAutoPin({ stick: false, geom: up, lastWriteTop: 480, readerMovedSinceWrite: false })
          .pin
      ).toBe(false)
      const r = evaluateAutoPin({
        stick: true,
  ...
  it('a 1px jitter at the bottom is within epsilon and keeps following', () => {
    ...
    const r = evaluateAutoPin({
      ...
      geom: jitter,
      lastWriteTop: 600,
      epsilon: SELF_SCROLL_EPSILON,
    })
    expect(r.stick).toBe(true)
    expect(r.pin).toBe(true)
  })

  it('bounded grid: sticking and no scroll-up preserve follow across growth', () => {
    for (let lastWriteTop = 0; lastWriteTop <= 5000; lastWriteTop += 97)
      for (let growth = 0; growth <= 5000; growth += 131) {
        const geom = {
          scrollTop: lastWriteTop,
          scrollHeight: lastWriteTop + 400 + growth,
          clientHeight: 400,
        }
        expect(evaluateAutoPin({ stick: true, geom, lastWriteTop }).stick).toBe(true)
      }
  })

  it('bounded grid: every upward move past epsilon releases follow', () => {
    for (let lastWriteTop = 100; lastWriteTop <= 5000; lastWriteTop += 97)
      for (let up = SELF_SCROLL_EPSILON + 1; up <= 100; up += 1) {
        const geom = {
          scrollTop: lastWriteTop - up,
          scrollHeight: lastWriteTop + 400,
          clientHeight: 400,
        }
        const result = evaluateAutoPin({ stick: true, geom, lastWriteTop })
        expect(result.stick).toBe(false)
...
describe('evaluateAutoPin — a restore owns the position', () => {
  ...
  it('refuses the pin while a restore holds the position', () => {
    expect(evaluateAutoPin({ stick: true, geom, lastWriteTop: 3091, restoreGate: true }).pin).toBe(
      false
    )
  })
  ...
  it('RELEASES follow rather than merely skipping it', () => {
    ...
    expect(
      evaluateAutoPin({ stick: true, geom, lastWriteTop: 3091, restoreGate: true }).stick
    ).toBe(false)
  })

  it('pins normally when no restore is in flight', () => {
    expect(evaluateAutoPin({ stick: true, geom, lastWriteTop: 3091, restoreGate: false }).pin).toBe(
      true
    )
  })
})
